...
export const getSandpackConfig = (theme = "light", template = "react") => {
  return {
    ...
    files:
      SANDPACK_TEMPLATES[template]?.files || SANDPACK_TEMPLATES.react.files,
    dependencies:
      SANDPACK_TEMPLATES[template]?.dependencies ||
      SANDPACK_TEMPLATES.react.dependencies,
  };
};
...
export const validateSandpackFiles = (files) => {
  ...
  const hasMainFile = Object.keys(files).some(
    (fileName) =>
  ...
  if (!hasMainFile) {
    warnings.push(
      "No main entry point found. Consider creating App.jsx or index.jsx"
    );
  }
  ...
  if (!hasReactImport) {
    warnings.push(
      "No React imports found. Make sure to import React in your components"
    );
  }
